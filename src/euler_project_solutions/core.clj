...
(defn special-pythagorean-triplet
  "Find the product of the Pythagorean triplet abc for which a + b + c = target-sum."
  [target-sum]
  (let [possible-operands (for [a (range 1 1001)
                                b (range 1 1001)
                                c (range 1 1001)
                                :when (and (< a b c) (= target-sum (+ a b c)))]
                            [a b c])
        triplets (filter #(let [a (nth % 0) b (nth % 1) c (nth % 2)]
